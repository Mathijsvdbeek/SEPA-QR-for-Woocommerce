...
define('QUERY_PARAM','mxp_qr'); // name of the query param (only needed if qr link is used)
define('TRANSIENT_PREFIX', 'MXPQR_'); // identify our transients or cache entries if the need to be deleted
// activate USE_TRANSIENTS if longer lifetime beyond current request is desired ,
// otherwise wp-cache is used which will not persist across requests without a persistent caching plugin
// define('USE_TRANSIENTS',true);  
define('TRANSIENT_LIFETIME', 3 * DAY_IN_SECONDS); //  for transients only, useless used for nonpersistent cache
...
function mxp_add_text_to_thankyoupage($order_id) {
...
add_action( 'woocommerce_thankyou_bacs', 'mxp_add_text_to_thankyoupage' );
...
add_action( 'woocommerce_email_after_order_table', 'mxp_email_after_order_table', 10, 4 );
...
function mxp_qrpage_from_hash($template) {
    global $wp_query;

    if(!isset( $wp_query->query[QUERY_PARAM] ) || $wp_query->query[QUERY_PARAM] == '') {
		// stop processing if our param does not exist or is empty
        return $template;
	}
    
	$md5 = $wp_query->query[QUERY_PARAM];

	if ( false === mxp_valid_md5($md5) ) { 
		$html = '<p>This is not a valid md5 hash :-(<br>Generating uncached dummy QR for 11 Euro/OrderID 11<br><img src="' . mxp_create_qrcode(11,11) . '"></p>';
	} elseif ( false === (  $qrcode = mxp_get_transient_by_hash($md5)  ) ) { 
		...
		if ( $md5 == "351436ef4b279e1811a6c68a2dd58b1b" ) { 
			...
			mxp_set_transient(11,11,SAD_SMILEY); 
		}
...
add_action('template_redirect', 'mxp_qrpage_from_hash');
...
function mxp_add_qv($vars){
    $vars[] = QUERY_PARAM;
    return $vars;
}
add_filter( 'query_vars', 'mxp_add_qv');
...
function mxp_get_qrcode($amount,$orderid) {

	if ( false === ( $qrcode = mxp_get_transient_by_data($amount,$orderid) ) ) {
		// not found - so regenerate the code  and save as transient
		$qrcode = mxp_create_qrcode($amount,$orderid);
	}
	if ( empty($qrcode) ) {
		return false;
	}
	
  mxp_set_transient( $amount,$orderid, $qrcode );	
  return $qrcode;
}
...
function mxp_create_qrcode ($amount,$orderid) {

 $payloadtext = PURPOSE_PREFIX . ' ' . $orderid;
...
function mxp_get_transient_by_hash ($md5) {
	  $transient = TRANSIENT_PREFIX . $md5;
	  if (defined('USE_TRANSIENTS')) { 
		  return get_transient( $transient );
	  }
      return wp_cache_get( $transient );
...
function mxp_set_transient ($amount,$id,$qrcode) {
	  $transient = TRANSIENT_PREFIX . md5($amount . '_' . $id);
	  if (defined('USE_TRANSIENTS')) { 
		$store = set_transient( $transient, $qrcode, TRANSIENT_LIFETIME );
	  } else {
	  	$store = wp_cache_set( $transient, $qrcode, '', TRANSIENT_LIFETIME );
	  }
	  return $store;
}
